...
setInterval(function () {
  ...
  iframe.contentWindow.postMessage("getLocationHref", iframe.src);
}, 1000);
...
window.addEventListener("message", (event) => {
  if (
    event.origin !== "https://dykarna.plingot.com" &&
    event.origin !== "https://dykarna.plingot.com" &&
    event.origin !== "https://dykarna.plingot.com" &&
    event.origin !== "https://dykarna.plingot.com"
  ) {
  ...
  const iframeLocationHref = event.data.href;
...
function extractValue(url) {
  const startIndex = url.indexOf("/event/") + 7;
...
function onIframeUrlChange(newUrl) {
  if (newUrl.includes("event/")) {
    if (location.search === "") {
      const eventId = extractValue(newUrl);
